      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between h-20">
          ...
          <Link to="/" className="flex items-center space-x-2">
            ...
            <h1 className="text-white hover:text-red-500 text-2xl font-semibold">Task Manager</h1>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-6">
            {["Home", "Tasks", "About"].map((item) => (
              <Link
                key={item}
                to={`/${item.toLowerCase()}`}
                className="text-white hover:text-red-500 px-5 py-3 rounded-lg text-lg font-medium transition ease-in-out duration-300"
              >
                {item}
              </Link>
            ))}
            ...
            <div className="flex flex-col items-center py-4 space-y-3">
              {["Home", "Tasks", "About"].map((item) => (
                <Link
                  key={item}
                  to={`/${item.toLowerCase()}`}
                  className="block text-white hover:text-red-500 px-6 py-3 rounded-md text-lg font-medium transition duration-300"
